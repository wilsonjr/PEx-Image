...
public class KNN {
    ...
    public Pair[][] execute(DistanceMatrix dmat) throws IOException {
        ...
        Logger.getLogger(this.getClass().getName()).log(Level.INFO, "KNN time: {0}s", (finish - start) / 1000.0f);
    ...
    public Pair[][] execute(ArrayList<OverlapRect> rects) throws IOException {
        ...
        for (int i = 0; i < rects.size(); i++) {
            for (int j = 0; j < rects.size(); j++) {
                ...
                float dist = (float) br.com.methods.utils.Util.distanciaEuclideana(rects.get(i).getUX(), rects.get(i).getUY(), 
                                                                           rects.get(j).getUX(), rects.get(j).getUY());
